...
******
1. Un dels grans problemes dels joves és la dificultat de comprar una propietat degut als alts preus de compra i de lloguer.  L'objectiu d'aquesta pràctica serà identificar les variables més determinants a l'hora de fixar el preu a una casa 
i posteriorment fer una predicció dels preus segons les variables de les que disposem. Es treballara amb un algoritme supervisat i un no supervisat per comparar els resultas.
...
Un dels grans problemes que em trobo jo com a jove es el gran preu dels lloguers i de les propietats per aixó he decidit fer aquest estudi. Primer de tot vaig buscar un joc de dades de la ciutat de Barcelona però no vaig trobar
...
# 
# # Agregar información de clusters a los datos
# PropertyPrices_numeric$cluster <- as.factor(PropertyPrices_numeric$cluster)
# 
# # Visualización de los clusters vs. valores reales
# ggplot(PropertyPrices_numeric, aes(x = price, color = cluster)) +
#   geom_point() +
#   labs(title = "Clusters vs. Valores Reales",
#        x = "Precio",
#        y = "Frecuencia") +
#   theme_minimal()
# 
